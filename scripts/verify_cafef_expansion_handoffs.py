...
def verify_archive(path,index,universe,contract):
    with zipfile.ZipFile(path) as z:
        ...
        if any(manifest.get(k)!=assignment.get(a) for k,a in (("expansion_id","expansion_id"),("assignment_id","assignment_id"),("worker_id","worker_id"),("git_commit","git_commit"),("execution_version","execution_version"),("assignment_sha256","assignment_sha256"),("universe_sha256","universe_hash"),("crawl_contract_sha256","crawl_contract_hash"))): raise ValueError("manifest consistency mismatch")
        ...
        if (manifest.get("adapter_version")!=ADAPTER_VERSION or manifest.get("run_id")!=run.get("run_id")
