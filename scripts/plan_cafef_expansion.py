...
def main(argv=None):
    ...
    contract={"expansion_id":EXPANSION_ID,"execution_version":EXECUTION_VERSION,"history_policy":HISTORY_POLICY,"source":"cafef","endpoint":TRADE_HISTORY_ENDPOINT,"page_size":PAGE_SIZE,"concurrency":1,"minimum_interval_seconds":3.0,"timeout_seconds":20,"attempts":2,"retry_http_statuses":sorted(RETRYABLE),"hard_stop_http_statuses":sorted(HARD_STOP),"hard_stop_markers":["CAPTCHA","Cloudflare","managed challenge","access denied"],"max_response_bytes":5000000,"maximum_pages_per_ticker":100,"target_start":TARGET_START,"target_collection_end":TARGET_COLLECTION_END,"comparison_snapshot":COMPARISON_SNAPSHOT,"safety_overlap_pages":1,"normalization":"CENTRAL_C8_ONLY","null_to_zero":False,"zero_volume_observation":"OBSERVED_ZERO_VOLUME","provider_boundary_semantics":"EMPTY_OR_SHORT_HISTORY_IS_NEUTRAL_PROVIDER_EVIDENCE_NOT_SUSPENSION_OR_NOT_LISTED"}
    write_json(out/"crawl_contract.json",contract)
    universe={"expansion_id":EXPANSION_ID,"execution_version":EXECUTION_VERSION,"history_policy":HISTORY_POLICY,"selection_frozen_pre_crawl":True,"target_count":600,"target_start":TARGET_START,"target_collection_end":TARGET_COLLECTION_END,"comparison_snapshot":COMPARISON_SNAPSHOT,"discovery_evidence":evidence,"importance_evidence":"UNAVAILABLE_NOT_INVENTED","securities":plan["selected"]}
    ...
    write_json(out/"index.json",{"expansion_id":EXPANSION_ID,"execution_version":EXECUTION_VERSION,"history_policy":HISTORY_POLICY,"git_branch":"m1-cafef-primary-experiment","git_commit_contract":"FROZEN_EXECUTION_VERSION_AVOIDS_RECURSIVE_COMMIT_HASH","universe_sha256":uh,"crawl_contract_sha256":ch,"expected_assignment_count":5,"assignments":assignments})
