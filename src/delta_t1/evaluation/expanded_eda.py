...
def _generate_eda_markdown(
    ...
    lines.extend([
        ...
        f"| &nbsp;&nbsp;+ Thiếu 1 – 5 phiên | {s['session_coverage_breakdown']['group_1_missing_1_to_20_days']['missing_1_to_5_days']} | {s['session_coverage_breakdown']['group_1_missing_1_to_20_days']['missing_1_to_5_days'] / total_sec:.1%} | Cohort near-ready cho A5-R2; không được nới 100% observation rule hay tạo zero-return row. |",
        f"| &nbsp;&nbsp;+ Thiếu 6 – 20 phiên | {s['session_coverage_breakdown']['group_1_missing_1_to_20_days']['missing_6_to_20_days']} | {s['session_coverage_breakdown']['group_1_missing_1_to_20_days']['missing_6_to_20_days'] / total_sec:.1%} | Cohort near-ready cho A5-R2; chỉ real row qua contract approved mới cải thiện readiness. |",
    ...
    lines.extend([
        ...
        "---",
        "",
        "## 4. Nhóm near-ready cho A5-R2 (không phải ứng viên nới lỏng chính sách)",
        "",
        ...
        "",
        "2. **Trạng thái methodology M2:**",
        "   - Giữ 100% required real observations; không forward/back-fill, interpolation, previous-close substitution, missing-to-zero hay zero-return imputation.",
        "   - Nhóm near-ready chỉ là input cho A5-R2. Market-feature-ready sau remediation phải được đo từ real provider rows theo contract approved; không đặt mục tiêu suy diễn trước.",
        "",
    ])
    return "\n".join(lines)
